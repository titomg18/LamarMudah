...
@section('title', 'Daftar Akun Baru - LamarMudah')
...
<div class="space-y-6">
    ...
    <div class="bg-slate-100/90 p-1.5 rounded-2xl flex items-center justify-between text-xs font-bold border border-slate-200/60 shadow-inner">
        <a href="{{ route('login') }}" class="w-1/2 text-slate-500 hover:text-slate-900 py-2.5 px-4 text-center transition-all font-semibold">
            Masuk
        </a>
        <a href="{{ route('register') }}" class="w-1/2 bg-white text-slate-900 py-2.5 px-4 rounded-xl shadow-sm text-center border border-slate-200/50 transition-all font-bold">
            Daftar Akun Baru
        </a>
    </div>
    ...
    <form action="{{ route('register') }}" method="POST" class="space-y-3.5">
        ...
        <!-- Full Name Field -->
        <div>
            <label for="name" class="text-xs font-bold text-slate-700 mb-1 block">Nama Lengkap</label>
            <div class="relative">
                ...
                <input type="text" name="name" id="name" required placeholder="Nama Lengkap Anda"
                       class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-600 focus:border-teal-600 text-sm text-slate-900 placeholder-slate-400 font-medium transition-all shadow-sm bg-slate-50/50 focus:bg-white">
            </div>
        </div>

        <!-- Email Field -->
        <div>
            <label for="email" class="text-xs font-bold text-slate-700 mb-1 block">Alamat Email</label>
            <div class="relative">
                <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div>
            <label for="phone" class="text-xs font-bold text-slate-700 mb-1 block">Nomor Telepon / WhatsApp</label>
            <div class="relative">
                <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <input type="tel" name="phone" id="phone" required placeholder="081234567890"
                       class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-600 focus:border-teal-600 text-sm text-slate-900 placeholder-slate-400 font-medium transition-all shadow-sm bg-slate-50/50 focus:bg-white">
            </div>
        </div>
        ...
        <!-- Password Field -->
        <div>
            <label for="password" class="text-xs font-bold text-slate-700 mb-1 block">Kata Sandi</label>
            <div class="relative">
                <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <input type="password" name="password" id="password" required placeholder="Minimal 8 karakter"
                       class="w-full pl-10 pr-10 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-600 focus:border-teal-600 text-sm text-slate-900 placeholder-slate-400 font-medium transition-all shadow-sm bg-slate-50/50 focus:bg-white">
        ...
        <div class="flex items-start gap-2.5 pt-1">
            <input type="checkbox" id="terms" name="terms" required class="w-4 h-4 mt-0.5 rounded text-teal-700 focus:ring-teal-600 border-slate-300 cursor-pointer" checked>
            <label for="terms" class="text-xs text-slate-600 font-medium cursor-pointer leading-tight">
                Saya menyetujui <a href="#" class="font-bold text-slate-800 hover:underline">Syarat & Ketentuan</a> serta <a href="#" class="font-bold text-slate-800 hover:underline">Kebijakan Privasi</a> LamarMudah.
            </label>
        </div>
